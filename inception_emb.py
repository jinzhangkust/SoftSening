...
class Img_Inception(nn.Module):
    ...
    def __init__(self, num_classes=1, aux_logits=False, transform_input=False):
        ...
        self.regressor =  nn.Sequential(
            #nn.AvgPool2d(kernel_size=8), # global average pooling just before the final layer
            nn.Dropout(),
            nn.Linear(2051, 1024), # one-output --> regression;    multi-output --> classification
    ...
    def forward(self, x):
        x = self.features(x)
        #print(x.shape)
        x = F. avg_pool2d(x, kernel_size=8)  # global average pooling just before the final layer
        x = x.view(x.size(0), -1)
        return x
